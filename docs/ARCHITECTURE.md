...
`Renderer` draws the full game to one canvas. It computes a fixed logical board from maze dimensions and tile size, centers the board in the viewport, fills the canvas black, draws wall tiles white, draws the goal green, draws the player blue, and overlays a simple win message.
...
The player is a circle and walls are axis-aligned rectangles. Movement is continuous. The player moves one axis at a time so blocked diagonal movement can still slide along open walls. Collision uses circle-vs-rectangle checks against nearby wall tiles, including corners, to prevent clipping.
...
4. Convert walkable cells into a tile map with wall borders and open floor.
5. Validate the result and retry with a different seed when constraints fail.

Solvability is guaranteed by preserving the carved start-to-goal path. Validation still independently confirms the path exists.
